/*
 * opencog/atoms/tcpsock/TcpSocketNode.cc
 *
...
#include "TcpSocketNode.h"

using namespace opencog;

TcpSocketNode::TcpSocketNode(Type t, const std::string&& url) :
	TextStreamNode(t, std::move(url)),
	_client_fd(-1),
	_listen_fd(-1),
	_port(-1)
{
	OC_ASSERT(nameserver().isA(_type, TCP_SOCKET_NODE),
		"Bad TcpSocketNode constructor!");
}

TcpSocketNode::TcpSocketNode(const std::string&& url) :
	TextStreamNode(TCP_SOCKET_NODE, std::move(url)),
	_client_fd(-1),
	_listen_fd(-1),
	_port(-1)
{
...
/// Open a TCP socket and start listening for connections.
///
/// The URI format is:
/// tcp://host:port
///
/// This creates the socket, binds to the given address/port, and starts
/// listening with a backlog of 1. It does NOT block waiting for a
/// client to connect; the accept() is deferred to the first call
/// to do_read() or do_write(). This allows the user to connect with
/// socat at their leisure.
///
void TcpSocketNode::open(const ValuePtr& vty)
{
	...
	if (0 != url.compare(0, 6, "tcp://"))
		throw RuntimeException(TRACE_INFO,
			"Unsupported URL \"%s\"\n"
			"Expected format: tcp://host:port\n",
			url.c_str());

	// Extract host and port. "tcp://" is 6 chars.
	...
	printf("Connect with: socat - TCP:%s:%d\n", host.c_str(), _port);
}
...
void TcpSocketNode::do_accept(void) const
{
	...
	if (0 > cfd)
	{
		...
		throw RuntimeException(TRACE_INFO,
			...
			_name.c_str(), norr, strerror(norr));
	}
...
void TcpSocketNode::close(const ValuePtr&)
{
...
void TcpSocketNode::barrier(AtomSpace* ignore)
{
	// Raw fd I/O has no user-space buffer to flush.
}

bool TcpSocketNode::connected(void) const
{
...
std::string TcpSocketNode::do_read(void) const
{
...
void TcpSocketNode::do_write(const std::string& str)
{
	// If no client yet, accept one (blocks until a client connects).
	{
		std::lock_guard<std::mutex> lock(_mtx);
